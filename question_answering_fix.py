...
class QuestionAnsweringModel:
    ...
    def calculate_results(self, truth, predictions, **kwargs):
        truth_dict = {}
        questions_dict = {}
        type_qas_dict = {}  # Lưu type_qas cho mỗi câu hỏi
        answers_list_dict = {}  # Lưu tất cả câu trả lời
        for item in truth:
            for qa in item.get("qas", []):  # Truy xuất qas trực tiếp, như code cũ
                q_id = qa["id"]
                questions_dict[q_id] = qa["question"]
                type_qas_dict[q_id] = qa.get("type_qas", "unknown")  # Lấy type_qas, mặc định là "unknown"
                if qa.get("answers"):
                    answers_list_dict[q_id] = [ans["text"] for ans in qa["answers"]]
                    truth_dict[q_id] = answers_list_dict[q_id][0]  # Lấy câu trả lời đầu tiên để tương thích
                else:
                    answers_list_dict[q_id] = [""]
                    truth_dict[q_id] = ""

        print(f"truth_dict size: {len(truth_dict)}")
        print(f"predictions size: {len(predictions)}")
        print(f"type_qas_dict: {type_qas_dict}")
        ...
        predicted_answers = []
        true_answers = []
        type_qas_stats = {}  # {type_qas: {"correct": int, "similar": int, "incorrect": int, "total": int}}

        for q_id in truth_dict:
            predicted_answer = predictions.get(q_id, "").strip()
            ground_truths = [ans.strip() for ans in answers_list_dict[q_id]]
            predicted_answers.append(predicted_answer)
            true_answers.append(ground_truths[0])  # Dùng câu trả lời đầu tiên cho extra_metrics

            type_qas = type_qas_dict[q_id]
            if type_qas not in type_qas_stats:
                type_qas_stats[type_qas] = {"correct": 0, "similar": 0, "incorrect": 0, "total": 0}

            type_qas_stats[type_qas]["total"] += 1

            # So sánh với tất cả ground truths
            is_correct = any(predicted_answer == gt for gt in ground_truths)
            is_similar = any(predicted_answer in gt or gt in predicted_answer for gt in ground_truths) if not is_correct else False

            if is_correct:
                correct += 1
                correct_text[q_id] = predicted_answer
                type_qas_stats[type_qas]["correct"] += 1
            elif is_similar:
                similar += 1
                similar_text[q_id] = {
                    "truth": ground_truths,
                ...
                type_qas_stats[type_qas]["similar"] += 1
            else:
                incorrect += 1
                incorrect_text[q_id] = {
                    "truth": ground_truths,
                ...
                type_qas_stats[type_qas]["incorrect"] += 1

        print(f"type_qas_stats: {type_qas_stats}")

        # Tính tỉ lệ lỗi sai cho mỗi type_qas
        type_qas_error_rates = {
            type_qas: stats["incorrect"] / stats["total"] if stats["total"] > 0 else 0.0
            for type_qas, stats in type_qas_stats.items()
        }
        ...
        result = {
            ...
            "similar": similar,
            "incorrect": incorrect,
            "type_qas_stats": type_qas_stats,
            "type_qas_error_rates": type_qas_error_rates,
            **extra_metrics,
        }
